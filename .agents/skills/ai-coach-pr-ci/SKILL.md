...
- Resolve current PR head and base first. Do not infer completion from green checks or auto-merge alone.
...
- For Sonar, query the target branch explicitly (`branch=develop`) or the PR key.
- Report `head`, `base`, checks, review threads, ruleset blockers, merge state, and ancestry as compact `path/status` facts.
- After a rebase, compare `git diff origin/develop...HEAD` and validate the current head again.
