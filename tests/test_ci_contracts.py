...
class CodexReviewWorkflowTests(unittest.TestCase):
    ...
    def test_codex_gate_uses_subscription_review_and_required_check_context(self):
        ...
        self.assertIn("const freshReviewRequired = context.payload.action === 'synchronize' && !manualReviewRequestedAt", workflow)
        self.assertIn("matrix.freshReviewRequired == true", workflow)
        self.assertIn("matrix.freshReviewRequired != true", workflow)
