...
class CodexReviewWorkflowTests(unittest.TestCase):
    ...
    def test_codex_gate_uses_subscription_review_and_required_check_context(self):
        ...
        self.assertIn("const releasePromotionPattern = /^chore\\/release-promotion-(\\d+\\.\\d+\\.\\d+)$/;", workflow)
        self.assertIn("pullRequest.base?.ref === 'main'", workflow)
        self.assertIn("pullRequest.title === `chore(release): promote ${promotionMatch[1]} to main`", workflow)
        ...
        self.assertIn("(promotionMatch && pullRequest.data.base.ref !== 'main')", workflow)
        self.assertIn("The trusted release bot owns this exact release promotion pull request", workflow)
        ...
        self.assertIn("reviewRequestedAt: reviewBaselineChanged", workflow)
