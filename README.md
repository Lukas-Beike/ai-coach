...
merged into `develop`, the workflow opens a promotion PR from `develop` to
protected `main`. When that PR is merged, the workflow creates a release tag
matching `APP_VERSION` on the resulting `main` commit. Its release notes
