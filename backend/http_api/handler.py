...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.dependencies.post_dispatcher.handle_before_maintenance(self, path, session):
                return
            with self.dependencies.maintenance_gate.operation():
                self.dependencies.post_dispatcher.handle_authenticated(self, path, session)
        except AppError as exc:
            if exc.status >= 500:
                self.dependencies.logger.exception(
                    exc.message,
    ...
    def send_sse_headers(self, *, persistent: bool = True) -> None:
        self.dependencies.response_transport.send_sse_headers(self, persistent=persistent)

    def send_sse_event(self, event: str, payload: Any, event_id: int | None = None) -> None:
        self.dependencies.response_transport.send_sse_event(self, event, payload, event_id)
    ...
    def send_bytes(self, status: int, data: bytes, content_type: str, headers: dict[str, str | list[str]] | None = None) -> None:
        self.dependencies.response_transport.send_bytes(self, status, data, content_type, headers)
