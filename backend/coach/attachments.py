...
def _gpx_point(node):
    ...
    elevation = next((float(child.text) for child in node if _gpx_local_name(child.tag) == "ele"), None)
    if elevation is not None and (not math.isfinite(elevation) or abs(elevation) > 20000):
        raise ValueError("Invalid elevation")
    return [lat, lon, elevation]


def _gpx_segment_stats(group):
    distance = ascent = descent = 0.0
    points = []
    previous = None
    for node in group:
        if _gpx_local_name(node.tag) not in {"trkpt", "rtept"}:
            continue
        point = _gpx_point(node)
        if previous:
            a, b = math.radians(previous[0]), math.radians(point[0])
            h = math.sin((b-a)/2)**2 + math.cos(a)*math.cos(b)*math.sin(math.radians(point[1]-previous[1])/2)**2
            distance += 6371000 * 2 * math.asin(math.sqrt(min(1, max(0, h))))
            if point[2] is not None and previous[2] is not None:
                difference = point[2] - previous[2]
                ascent += max(0, difference)
                descent += max(0, -difference)
        points.append(point)
        previous = point
    return distance, ascent, descent, points
...
def gpx_summary(data):
    ...
    if _gpx_local_name(root.tag) != "gpx":
        raise ValueError("Not GPX")
    groups = [node for node in root.iter() if _gpx_local_name(node.tag) in {"trkseg", "rte"}]
    distance = ascent = descent = 0.0
    points = []
    for group in groups:
        segment_distance, segment_ascent, segment_descent, segment_points = _gpx_segment_stats(group)
...
def _decode_attachment(item):
    ...
    return name, encoded, data


def _structured_attachment(name, encoded, data):
    if name.lower().endswith(".gpx"):
        ...
        return {"name": name, "type": "gpx", "mime": "application/gpx+xml", "gemini_mime": "text/xml",
    ...
    if name.lower().endswith(".fit"):
        ...
        return {"name": name, "type": "fit", "mime": "application/octet-stream", "gemini_mime": "application/octet-stream",
                "data": encoded, "summary": summary}
    return None


def _image_mime(name, data):
    signatures = (
        (b"\x89PNG\r\n\x1a\n", (".png",), "image/png"),
        (b"\xff\xd8\xff", (".jpg", ".jpeg"), "image/jpeg"),
        (b"RIFF", (".webp",), "image/webp"),
    )
    for signature, suffixes, mime in signatures:
        if data.startswith(signature) and name.lower().endswith(suffixes):
            if mime != "image/webp" or data[8:12] == b"WEBP":
                return mime
    return None


def _validated_attachment(item):
    name, encoded, data = _decode_attachment(item)
    structured = _structured_attachment(name, encoded, data)
    if structured:
        return structured
    mime = _image_mime(name, data)
    if not mime:
        raise ValueError("Unsupported attachment")
    return {"name": name, "type": "image", "mime": mime, "data": encoded}
...
def validate_attachments(value):
    ...
    return [_validated_attachment(item) for item in value]
