...
def build_tool_contracts(
    *,
    ...
    COACH_STRUCTURED_TOOLS = [
        ...
        _canonical_coach_tool("start_provider_refresh", "Queue an explicitly requested read-only provider refresh. For a Garmin catch-up after an outage or when the athlete asks for a 30-day refresh, pass days=30; otherwise use the saved provider window.", {"days": {"type": "integer", "description": "Requested history window in days. Set to 30 for an explicitly requested Garmin catch-up."}, "reason": {"type": "string"}}),
