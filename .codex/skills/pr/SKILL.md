...
4. Enter a repeat-until-merged loop. Poll the PR, its checks, reviews, comments, and review threads in short intervals (about 30 seconds; never block updates for more than 60 seconds). Do not finish while the PR is merely open, auto-merge-enabled, or apparently complete. Before evaluating the checks on each iteration, reconcile all PR feedback:
   ...
   - After the clean-feedback gate passes, enable squash auto-merge with `gh pr merge <number> --auto --squash` on every iteration where the current head has no unresolved feedback, regardless of whether required checks are still pending or already passing. Re-enable it after every later push only after the same gate passes again.
   ...
   - Request exactly one Codex review by posting the PR comment `@codex review` after the PR is ready for review. The GitHub Action is comment-driven; do not expect PR events or pushes to start Codex automatically. After that review, fix its findings and reply to/resolve the related comments. Do not request another Codex review after ordinary findings or CI fixes. Request one additional review only when the previous Codex review contained a P1 finding; otherwise the fix and closing the comments are sufficient.
   - If checks are pending, wait and poll. Continue the loop after every fix, push, review response, thread resolution, rebase, and auto-merge state change. Do not stop at green checks, resolved comments, enabled auto-merge, or a bot's completion message: finish only after GitHub reports the PR as actually merged, with `mergedAt` and a merge commit, all final required checks green, every review thread resolved, and every top-level comment answered with its requested action complete. If GitHub reports a required review, permission, unavailable runner, merge queue, or policy requirement that the agent cannot satisfy, report it clearly and stop rather than bypassing it.
