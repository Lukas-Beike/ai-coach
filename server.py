...
def garmin_profile_max_hr(snapshot: dict[str, Any]) -> dict[str, float | int]:
    ...
    for item in _garmin_mapping_nodes(snapshot.get("heart_rate_zones")):
        max_hr = as_number(first_present(item, ("maxHeartRateUsed", "maxHeartRate", "maxHR")))
        if max_hr is not None and 80 <= float(max_hr) <= 260:
            values[_garmin_sport_category(first_present(item, ("sport", "sportType", "activityType")))].append(max_hr)
    return {kind: max(items) for kind, items in values.items() if items}


def _garmin_collect_vo2_values(value: Any, values: dict[str, list[float | int]], path: tuple[str, ...] = ()) -> None:
    if isinstance(value, dict):
        for key, item in value.items():
...
def _garmin_threshold_metrics(snapshot: dict[str, Any]) -> tuple[float | int | None, float | int | None, float | int | None, float | int | None, float | int | None]:
    running_threshold = snapshot.get("running_threshold")
    cycling_ftp = _garmin_bounded_metric(
        _garmin_last_numeric(snapshot.get("cycling_ftp"), {"functionalthresholdpower", "ftp", "cyclingftp"}), 50, 700,
    )
    running_power = _garmin_bounded_metric(
        _garmin_last_numeric(running_threshold, {"functionalthresholdpower", "ftp", "runningftp", "power"}), 50, 800,
    )
    running_pace = _garmin_pace_seconds(_garmin_last_value(
        running_threshold,
        {"speed", "speedinmeterspersecond", "speedmeterspersecond", "lactatethresholdspeed", "thresholdspeed", "pace", "paceinsecondsperkilometer", "thresholdpace"},
    ))
    running_hr = _garmin_bounded_metric(
        _garmin_last_numeric(running_threshold, {"heartrate", "hearrate", "heartraterunning", "lthr"}), 80, 230,
    )
    cycling_hr = _garmin_bounded_metric(_garmin_last_numeric(running_threshold, {"heartratecycling"}), 80, 230)
...
def _garmin_max_hr_samples(snapshot: dict[str, Any]) -> tuple[dict[str, float | int], dict[str, list[float | int]], list[Any]]:
    ...
    values: dict[str, list[float | int]] = {"cycling": [], "running": []}
    stored = snapshot.get("sport_max_hr") if isinstance(snapshot.get("sport_max_hr"), dict) else {}
    ...
    for kind in values:
        ...
        if profile_value is not None:
            values[kind].append(profile_value)
            continue
...
def _garmin_performance_units(
    weight: dict[str, Any], max_hr_values: dict[str, list[float | int]], profile_max_hr: dict[str, float | int],
    cycling_vo2: float | int | None, running_vo2: float | int | None, race_values: dict[str, float | int | None],
    thresholds: tuple[float | int | None, float | int | None, float | int | None, float | int | None, float | int | None],
...
def workout_event_payload(workout_id: str, workout: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "description": str(workout.get("description") or "")[:12000],
        "moving_time": round(structured_seconds) if structured_seconds is not None else duration * 60,
...
def validate_intervals_workout_result(workout: dict[str, Any], remote: dict[str, Any]) -> None:
...
def normalize_planned_unit(
    ...
    *,
    ...
    if not isinstance(workout, dict) or not isinstance(workout.get("sport"), str) or not workout["sport"].strip():
        raise AppError(400, "Eine geplante Einheit benoetigt ihre Sportart.", reason="invalid_workout")
    normalized = normalize_library_workout(
        {**workout, "type": intervals_workout_sport(workout["sport"]), "date": str(workout.get("date") or workout.get("start_date_local") or "")[:10]},
        local_id=local_id,
...
def create_local_library_template(workout: dict[str, Any]) -> dict[str, Any]:
    ...
    if not 5 <= duration <= 1440:
        raise AppError(400, "Die Vorlagendauer muss zwischen 5 und 1440 Minuten liegen.")
...
def _canonical_remote_indexes(remote_rows: list[dict[str, Any]]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    return (
        {str(item.get("id")): item for item in remote_rows if item.get("id") not in (None, "")},
        {str(item.get("external_id")): item for item in remote_rows if item.get("external_id")},
    )


def _canonical_linked_remote(
    entry: dict[str, Any], remote_by_id: dict[str, dict[str, Any]], remote_by_external: dict[str, dict[str, Any]],
) -> dict[str, Any] | None:
    ...
    if linked is None:
        ...
        linked = remote_by_external.get(remote_external_id) if remote_external_id else None
    return linked
...
def _validate_structured_plan_limits(payload: dict[str, Any]) -> None:
    workouts = payload.get("workouts")
    if not isinstance(workouts, list) or not workouts:
        raise AppError(400, "Ein Planartefakt benötigt mindestens eine Einheit.", reason="plan_limit")
    if len(workouts) > 366:
        raise AppError(400, "Ein Planartefakt darf höchstens 366 Einheiten enthalten.", reason="plan_limit")
    dates = []
    for workout in workouts:
        if not isinstance(workout, dict):
...
def _prepare_structured_training_change(change: dict[str, Any]) -> dict[str, Any]:
    ...
    missing_fields = [
        ...
        if field not in change or change[field] is None or (isinstance(change[field], str) and not change[field].strip())
    ]
    if missing_fields:
        raise AppError(400, "Eine neue geplante Einheit benötigt alle Workout-Felder.", reason="invalid_change")
    ...
    prepared = {"action": "create", **{key: normalized[key] for key in (
...
def _prepare_structured_training_changes(arguments: dict[str, Any]) -> list[dict[str, Any]]:
    changes = arguments.get("changes") or []
    if not isinstance(changes, list) or not changes or len(changes) > COACH_TRAINING_CHANGE_LIMIT:
        raise AppError(400, f"Ein Coach-Kommando darf höchstens {COACH_TRAINING_CHANGE_LIMIT} Änderungen enthalten.", reason="change_limit")
    prepared = []
    for change in changes:
        if not isinstance(change, dict):
            raise AppError(400, "Jede Planänderung muss ein Objekt sein.", reason="invalid_change")
        prepared.append(_prepare_structured_training_change(change))
    return prepared


def _validate_structured_training_revision(
    arguments: dict[str, Any], *, require_revision: bool, db: Any,
) -> int:
    revision_row = db.execute(SELECT_PLANNING_REVISION_SQL).fetchone()
...
def _structured_training_membership_update(
    change: dict[str, Any], db: Any,
) -> tuple[str | None, str | None]:
    action = str(change.get("action") or "update").strip().casefold()
    local_id = str(change.get("local_id") or "").strip()
    ...
    if _structured_training_change_moves_bounds(action, change, current) and membership:
        return membership, membership
    return membership, None


def _structured_training_change_moves_bounds(
    action: str, change: dict[str, Any], current: dict[str, Any],
) -> bool:
    if action in {"delete", "archive", "restore"}:
        return True
    return action == "update" and "date" in change and not str(change.get("date") or "").startswith(
        str(current.get("date") or "")[:10]
    )


def _collect_structured_training_memberships(
    changes: list[dict[str, Any]], db: Any,
) -> tuple[set[str], set[str]]:
    referenced_memberships: set[str] = set()
    plans_needing_bounds: set[str] = set()
    for change in changes:
        membership, bounds_membership = _structured_training_membership_update(change, db)
...
def _validate_structured_training_create_plan_ids(
    changes: list[dict[str, Any]], derived_plan: dict[str, str], plans_needing_bounds: set[str],
) -> None:
    for change in changes:
        if str(change.get("action") or "update").strip().casefold() != "create":
            continue
        ...
        if requested_plan_id and requested_plan_id != derived_plan.get("plan_id"):
            raise AppError(403, "Eine neue Einheit darf nur dem eindeutig abgeleiteten Plan zugeordnet werden.", reason="intent_scope_denied")
        if requested_plan_id or ("plan_id" not in change and derived_plan):
            plans_needing_bounds.add(derived_plan["plan_id"])


def _derive_structured_training_plan(
    changes: list[dict[str, Any]], authorized_plan_id: str | None, db: Any,
) -> tuple[dict[str, str], set[str]]:
    referenced_memberships, plans_needing_bounds = _collect_structured_training_memberships(changes, db)
    derived_plan = _resolve_structured_training_plan_reference(referenced_memberships, authorized_plan_id, db)
    _validate_structured_training_create_plan_ids(changes, derived_plan, plans_needing_bounds)
    return derived_plan, plans_needing_bounds


def _apply_structured_training_change_rows(
    changes: list[dict[str, Any]], derived_plan: dict[str, str], db: Any,
) -> list[dict[str, Any]]:
    applied = []
    for change in changes:
        if str(change.get("action") or "update").strip().casefold() == "create":
...
def current_coach_proposals(session_csrf_hash: str) -> list[dict[str, Any]]:
...
def coach_command_receipt(client_turn_id: Any, session_csrf_hash: str) -> dict[str, Any]:
    ...
    with DB_LOCK, database() as db:
        ...
        for proposal in proposals:
            ...
            if current:
                value = _coach_action_view(current)
